...
PixelShader::PixelShader(Graphics &gfx, const std::wstring &path)
{
    ...
    GFX_THROW_INFO(D3DReadFileToBlob(path.c_str(), &pPSBlob));
    GFX_THROW_INFO(GetDevice(gfx)->CreatePixelShader(pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), nullptr, &pPixelShader));
}